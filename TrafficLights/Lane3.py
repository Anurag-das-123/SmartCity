...
if __name__ == "__main__":
    ...
    while True:
        ...
        if flag == 1 and lane_id == "three":
            t = int(time.time())
            thread = Thread(target=thread_function, args=(27, "green", "black", "black"))
            thread1 = Thread(target=thread_function1, args=(27, ))
            thread.start()
            thread1.start()
            mes = {'laneId': 3, 'green': g, 'starttime': str(datetime.now())}
            wait = int(time.time()) + int(g)
            while time.time() < wait:
            ...
            myAWSIoTMQTTClient.publish("lane3/numCar", str(random.randint(0, int(g))), 0)
            flag = 0
        else:
            g = int(g)+1
